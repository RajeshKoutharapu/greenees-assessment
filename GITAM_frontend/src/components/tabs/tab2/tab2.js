...
const Tab2 = () => {
  ...
  const [generalInstrumentData, setGeneralInstrumentData] = useState([
    { instrument: '', time: '' }, // Ensure proper initial structure
  ]);
  
  const [mainInstrumentData, setMainInstrumentData] = useState([]);
  const [mainInstrumentTables, setMainInstrumentTables] = useState({});
